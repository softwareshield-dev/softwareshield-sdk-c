...
TEST_CASE("hard-date-valid-range", tag) {
    //entity params
    ...
    SECTION("license") {
        ...
        SECTION("inspector"){
            TLM_HardDate insp(lic.get());

            CHECK(insp.mode() == TLM_HardDate::Mode::VALID_RANGE);
            CHECK(insp.exitAppOnExpire());

            CHECK(insp.rollbackTolerance() == 4000);
            CHECK(insp.timeBegin().time_since_epoch() == std::chrono::seconds(T0));
            CHECK(insp.timeEnd().time_since_epoch() == std::chrono::seconds(T1));
        }
    }
...
TEST_CASE("hard-date-expire-after", tag) {
    //entity params
    ...
    SECTION("license"){
        ...
        SECTION("inspector"){
            ...
            CHECK(insp.mode() == TLM_HardDate::Mode::EXPIRE_AFTER);
            CHECK(insp.exitAppOnExpire());
            CHECK(insp.rollbackTolerance() == 4000);
            CHECK(insp.timeEnd().time_since_epoch() == std::chrono::seconds(T1));
        }
...
TEST_CASE("hard-date-valid-since", tag) {
    //entity params
    const std::time_t T0 = 1609488000; //2021/01/01 12:00:00 AM
    const char* entity_id = "8b8c1bb6-632c-46fb-a974-3009130b6028";
    ...
    SECTION("license"){
        ...
        SECTION("inspector"){
            ...
            CHECK(insp.mode() == TLM_HardDate::Mode::VALID_SINCE);
            CHECK(insp.exitAppOnExpire());
            CHECK(insp.rollbackTolerance() == 4000);
            CHECK(insp.timeBegin().time_since_epoch() == std::chrono::seconds(T0));
        }
    ...
    SECTION("actions"){
        ...
        std::string req_code = req->code();
        CHECK(req_code == "VIJM3-7UJLJ-MFHNP-CMKSW-6QGF3-I5A6H-D9Y33");
        CHECK_FALSE(req_code.empty());

        //license-code is generated by manual-activator
        const char* license_code = "EJBM-CB6Y-TGEP-S6KD-GHSV-4DFP-S9U6-BX4S-PNEP";

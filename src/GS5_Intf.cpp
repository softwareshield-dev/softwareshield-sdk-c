...
BIND_FUNC(41, const char *, gsGetActionString, TActionHandle hAct) hAct BIND_END
   ...
   */
    BIND_FUNC(42, int, gsGetActionParamCount, TActionHandle hAct) hAct BIND_END

    BIND_FUNC(43, TVarHandle, gsGetActionParamByName, TActionHandle hAct, const char *paramName) hAct,
    paramName
    BIND_END
    ...
    BIND_FUNC(50, TVarHandle, gsAddVariable, const char *varName, TVarType varType, int attr, const char *initValStr)
        varName,
    varType, attr, initValStr BIND_END

    BIND_FUNC(51, bool, gsRemoveVariable, const char *varName) varName BIND_END

    BIND_FUNC(52, TVarHandle, gsGetVariable, const char *varName) varName BIND_END

    BIND_FUNC(53, const char *, gsGetVariableName, TVarHandle hVar) hVar BIND_END

    BIND_FUNC(54, TVarType, gsGetVariableType, TVarHandle hVar) hVar BIND_END

    BIND_FUNC(55, const char *, gsVariableTypeToString, var_type_t paramType) paramType BIND_END

    BIND_FUNC(56, int, gsGetVariableAttr, TVarHandle hVar) hVar BIND_END

    BIND_FUNC(65, const char *, gsVariableAttrToString, int permit, char *buf, int bufSize) permit,
    buf, bufSize BIND_END

    BIND_FUNC(66, int, gsVariableAttrFromString, const char *permitStr) permitStr BIND_END

    //Value Get/Set
    BIND_FUNC(57, const char *, gsGetVariableValueAsString, TVarHandle hVar) hVar BIND_END

    BIND_FUNC(58, bool, gsSetVariableValueFromString, TVarHandle hVar, const char *valstr) hVar,
    valstr
    BIND_END
    ...
    BIND_FUNC(47, TActionHandle, gsAddRequestActionEx, TRequestHandle hReq, action_id_t actId, const char *entityId, const char *licenseId) hReq,
    actId, entityId, licenseId BIND_END

    BIND_FUNC(45, const char *, gsGetRequestCode, TRequestHandle hReq) hReq BIND_END

    BIND_FUNC(46, bool, gsApplyLicenseCode, const char *licenseCode) licenseCode BIND_END

    //---------- Time Engine Service ------------
    BIND_PROC0(70, gsTurnOnInternalTimer) BIND_PROC0(71, gsTurnOffInternalTimer) BIND_FUNC0(72, bool, gsIsInternalTimerActive) BIND_PROC0(73, gsTickFromExternalTimer) BIND_PROC0(74, gsPauseTimeEngine) BIND_PROC0(75, gsResumeTimeEngine) BIND_FUNC0(76, bool, gsIsTimeEngineActive)

    //Monitor
    BIND_FUNC(90, TMonitorHandle, gsCreateMonitorEx, gs5_monitor_callback cbMonitor, void *usrData, const char *monitorName) cbMonitor,
    usrData, monitorName BIND_END

    BIND_FUNC(86, int, gsGetEventId, TEventHandle hEvent) hEvent BIND_END

    BIND_FUNC(87, TEventType, gsGetEventType, TEventHandle hEvent) hEvent BIND_END

    BIND_FUNC(88, TEventSourceHandle, gsGetEventSource, TEventHandle hEvent) hEvent BIND_END

    //HTML
    BIND_FUNC(80, bool, gsRenderHTML, const char *url, const char *title, int width, int height) url,
    title, width, height BIND_END

    BIND_FUNC(83, bool, gsRenderHTMLEx, const char *url, const char *title, int width, int height, bool resizable, bool exitAppWhenUIClosed, bool cleanUpAfterRendering) url,
    title, width, height, resizable, exitAppWhenUIClosed, cleanUpAfterRendering BIND_END

    BIND_FUNC0(81, bool, gsRunInWrappedMode)

        BIND_FUNC(82, bool, gsRunInsideVM, vm_mask_t vmask) vmask BIND_END

    BIND_FUNC0(91, bool, gsIsDebugVersion)

        BIND_PROC(92, gsTrace, const char *msg) msg BIND_END

    BIND_FUNC(67, bool, gsIsVariableValid, TVarHandle hVar) hVar BIND_END

    //Application Control
    BIND_PROC(93, gsExitApp, int rc) rc BIND_END

    BIND_PROC(94, gsTerminateApp, int rc) rc BIND_END

    BIND_PROC0(95, gsPlayApp) BIND_PROC0(96, gsRestartApp) BIND_FUNC0(102, bool, gsIsRestartedApp)

        BIND_FUNC0(97, const char *, gsGetAppRootPath) BIND_FUNC0(98, const char *, gsGetAppCommandLine) BIND_FUNC0(101, const char *, gsGetAppMainExe)

    //Session Variables
    BIND_PROC(99, gsSetAppVar, const char *name, const char *val) name,
    val
    BIND_END
    ...
    BIND_FUNC(105, TLicenseHandle, gsCreateCustomLicense, const char *licId, const char *licName, const char *description, void *usrData,
        ...
        licId,
    licName, description, usrData, cbIsValid, cbStartAccess, cbFinishAccess, cbOnAction, cbDestroy BIND_END

    BIND_FUNC(106, bool, gsBindLicense, TEntityHandle hEntity, TLicenseHandle hLic) hEntity,
    hLic
    BIND_END
    ...
    BIND_PROC(108, gsRegisterCustomLicense, const char *licId, lm_create_callback createLM, void *usrData)
        licId,
    createLM, usrData BIND_END

    BIND_PROC(109, gsAddLicenseParamStr, TLicenseHandle hLic, const char *paramName, const char *initValue, int permission) hLic,
    paramName, initValue, permission BIND_END

    BIND_PROC(110, gsAddLicenseParamInt, TLicenseHandle hLic, const char *paramName, int initValue, int permission) hLic,
    paramName, initValue, permission BIND_END

    BIND_PROC(111, gsAddLicenseParamInt64, TLicenseHandle hLic, const char *paramName, int64_t initValue, int permission) hLic,
    paramName, initValue, permission BIND_END

    BIND_PROC(112, gsAddLicenseParamBool, TLicenseHandle hLic, const char *paramName, bool initValue, int permission) hLic,
    paramName, initValue, permission BIND_END

    BIND_PROC(113, gsAddLicenseParamFloat, TLicenseHandle hLic, const char *paramName, float initValue, int permission) hLic,
    paramName, initValue, permission BIND_END

    BIND_PROC(114, gsAddLicenseParamTime, TLicenseHandle hLic, const char *paramName, time_t initValue, int permission) hLic,
    paramName, initValue, permission BIND_END

    BIND_PROC(115, gsAddLicenseParamDouble, TLicenseHandle hLic, const char *paramName, double initValue, int permission) hLic,
    paramName, initValue, permission BIND_END
...
index
    BIND_END
        ...
        BIND_PROC(89, gsPostUserEvent, unsigned int evtId, bool bSync, void *usrData, unsigned int usrDataSize)
            evtId,
    bSync, usrData, usrDataSize BIND_END

    BIND_FUNC(124, void *, gsGetUserEventData, TEventHandle hEvent, unsigned int *usrDataSize) hEvent,
    usrDataSize
    BIND_END
...
timeout
    BIND_END
        ...
        BIND_PROC(132, gsIsServerAliveAsync, ping_cb pcb, void *userData, int timeout)
            pcb,
    userData, timeout BIND_END

    BIND_FUNC(133, bool, gsApplySN, const char *sn, int *pRetCode, const char **ppSNRef, int timeout) sn,
    pRetCode, ppSNRef, timeout BIND_END

    BIND_PROC(134, gsApplySNAsync, const char *sn, activate_cb activateCB, void *userData, int timeout) sn,
    activateCB, userData, timeout BIND_END

    //136
    BIND_FUNC(136, bool, gsHasLicense, TEntityHandle hEntity) hEntity BIND_END

    //137
    BIND_FUNC(137, TLicenseHandle, gsOpenLicense, TEntityHandle hEntity) hEntity BIND_END

    //138
    BIND_PROC(138, gsLockLicense, TLicenseHandle hLic) hLic BIND_END

    //139
    BIND_FUNC(139, bool, gsIsSNValid, const char *sn, int timeout) sn,
    timeout
    BIND_END
    ...
    BIND_PROC(140, gsIsSNValidAsync, const char *sn, testsn_cb cb, void *userData, int timeout)
        sn,
    cb, userData, timeout BIND_END

    //135
    BIND_FUNC(135, bool, gsRevokeApp, int timeout, const char *sn) timeout,
    sn
    BIND_END
...
index
    ...
    //149
    BIND_FUNC(149, TMPHandle, gsMPOpen, const char *mpStr) mpStr BIND_END

    BIND_FUNC(141, bool, gsMPImportOnline, TMPHandle hMP, const char *sn, int timeout) hMP,
    sn, timeout BIND_END

    //150
    BIND_FUNC(150, const char *, gsMPGetImportOfflineRequestCode, TMPHandle hMP) hMP BIND_END
    //151
    BIND_FUNC(151, bool, gsMPImportOffline, TMPHandle hMP, const char *licenseCode) hMP,
    licenseCode
    BIND_END
    //152
...
hCodeExchange, productId, buildId, sn, requestCode BIND_END

                                           BIND_FUNC(161, int, gsCodeExchangeGetErrorCode, gs_handle_t hCodeExchange) hCodeExchange BIND_END

    BIND_FUNC(162, const char *, gsCodeExchangeGetErrorMessage, gs_handle_t hCodeExchange) hCodeExchange BIND_END

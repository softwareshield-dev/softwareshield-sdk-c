...
#define NORETURN __attribute__((noreturn))
  ...
  */
  ...
  template <> void TGSVariable::get<int>(int& v);
  /// gets value as 64-bit integer
  template <> void TGSVariable::get<int64_t>(int64_t& v);
  /// gets value as float
  template <> void TGSVariable::get<float>(float& v);
  /// gets value as double
  template <> void TGSVariable::get<double>(double& v);
  /// gets value as string
  template <> void TGSVariable::get<std::string>(std::string& v);

  /// sets value from 32-bit integer
  template <> void TGSVariable::set<int>(int v);
  /// sets value from 64-bit integer
  template <> void TGSVariable::set<int64_t>(int64_t v);
  /// sets value from float
  template <> void TGSVariable::set<float>(float v);
  /// sets value from double
  template <> void TGSVariable::set<double>(double v);
  /// sets value from string
  template <> void TGSVariable::set<std::string>(std::string v);
  //@}
  ...
  */
  ...
  public:
    ...
    * \return the pointer to action object, NULL if the action type id is not supported.
    */
    DEPRECATED TGSAction* addAction(action_id_t actId, TGSLicense * license);
    /** \brief adds an action targeting a single license object
    *
    * \param actId Action type id;
    ...
    * \return the pointer to action object, NULL if the action type id is not supported.
    */
    TGSAction* addAction(action_id_t actId, const char* entityId);
  ...
  */
  ...
  private:
    ...
    static void WINAPI s_pingCB(bool ok, void * userData){
      std::auto_ptr<TParamCopy<T> > param((TParamCopy<T>*)userData);
      param->_fcb(ok);
    }
  ...
  public:
    ...
    */
    ...
    static TCodeExchange* beginCodeExchange(){
      ...
      if (h != NULL) return new TCodeExchange(h);
      return NULL;
    }
  };
